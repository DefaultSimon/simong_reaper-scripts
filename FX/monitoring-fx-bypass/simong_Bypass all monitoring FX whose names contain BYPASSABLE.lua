...
for _, monitoring_fx in ipairs(monitoring_effects) do
    ...
    if is_configured_as_toggleable == true then
        ...
        if is_enabled then
            reaper.TrackFX_SetEnabled(master_track, monitoring_fx.effective_index, false)
        end
    end
end
...
lib.end_project_undo_block(current_project, "Action: Bypass all monitoring FX whose names contain [BYPASSABLE]")
